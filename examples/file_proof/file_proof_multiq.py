...
def setup_bbc_client():
    bbc_app_client = bbc_app.BBcAppClient(port=DEFAULT_CORE_PORT, multiq=True, loglevel="all")
    bbc_app_client.set_user_id(user_id)
    bbc_app_client.set_domain_id(domain_id)
    # bbc_app_client.set_callback(MessageProcessor())
...
def store_proc(file, txid=None):
    ...
    if txid:
        query_id = bbc_app_client.search_transaction(txid)
        response_data = bbc_app_client.callback.sync_by_queryid(query_id)
        if response_data[KeyType.status] < ESUCCESS:
            print("ERROR: ", response_data[KeyType.reason].decode())
            sys.exit(0)
        prev_tx, fmt_type = bbclib.deserialize(response_data[KeyType.transaction_data])
    ...
    query_id = bbc_app_client.insert_transaction(store_transaction)
    assert query_id
    response_data = bbc_app_client.callback.sync_by_queryid(query_id)
    if response_data[KeyType.status] < ESUCCESS:
        print("ERROR: ", response_data[KeyType.reason].decode())
...
def enter_file_wait_mode():
    ...
    # wait_for_transaction_msg
    print("Waiting for file transfer...")
    response_data = bbc_app_client.callback.synchronize()
    if response_data[KeyType.status] < ESUCCESS:
        print("ERROR: ", response_data[KeyType.reason].decode())
        assert False
    query_id_signreq = response_data[KeyType.query_id]
    
    # pick_valid_transaction_info
    transaction ,fmt_type = bbclib.deserialize(response_data[KeyType.transaction_data])
    asset_files = response_data[KeyType.all_asset_files]
    asset_id = transaction.relations[0].asset.asset_id
    source_id = response_data[KeyType.source_user_id]
    transaction_id = transaction.transaction_id
    if asset_id not in asset_files:
        print("No valid files.")
        print(response_data)
        bbc_app_client.sendback_denial_of_sign(source_id, transaction_id)
        assert False
    
    file_data = asset_files[asset_id]
    file_digest = hashlib.sha256(file_data).digest()

    asset_file_digest_str = binascii.b2a_hex(transaction.relations[0].asset.asset_file_digest).decode()
    file_digest_str = binascii.b2a_hex(file_digest).decode()
    print(asset_file_digest_str)
    print(file_digest_str)

    # prompt_user_to_accept_the_file
    print("====> Do you want to accept the file?")
    answer = input('(Y/N) >> ')
    if answer != "Y":
        bbc_app_client.sendback_denial_of_sign(source_id, transaction_id, "Denied to accept the file")
    ...
    query_id = bbc_app_client.sendback_signature(dest_user_id=source_id, transaction_id=transaction_id, ref_index=-1, signature=signature, query_id=query_id_signreq)
    assert query_id

    # wait_for_file_info_msg
    print("Waiting for the message from the sender...")
    response_data = bbc_app_client.callback.synchronize(timeout=10)
    if response_data is None:
        print("No final message received... Ask the sender about the filename and transaction_id")
...
def enter_file_send_mode(filename):
    ...
    # search_reference_txid_from_mappings
    ref_txid = None
    file_info = get_id_from_mappings(filename, asset_group_id)
    if file_info:
        ref_txid = file_info['transaction_id']
    
    bbc_app_client = setup_bbc_client()

    # create_transaction_object_for_file_data
    ref_txids = [ref_txid]
    if ref_txids is None:
        ref_txids = []
    ...
    transaction = bbclib.make_transaction(relation_num=1, witness=True)
    bbclib.add_relation_asset(transaction, relation_idx=0, asset_group_id=asset_group_id,
                            user_id=user_id, asset_body=user_info_msg, asset_file=file_data)
    if ref_txid in ref_txids:
        query_id = bbc_app_client.search_transaction(ref_txid)
        assert query_id
        response_data = bbc_app_client.callback.sync_by_queryid(query_id)
        if response_data[KeyType.status] < ESUCCESS:
            print("ERROR: %s" % response_data[KeyType.reason].decode())
            assert False
        prev_tx, fmt_type = bbclib.deserialize(response_data[KeyType.transaction_data])
        bbclib.add_relation_pointer(transaction, relation_idx=0, ref_transaction_id=prev_tx.digest())
    
    asset_id = transaction.relations[0].asset.asset_id
    asset_files = {asset_id: file_data}
    query_id = bbc_app_client.gather_signatures(transaction, destinations=[receiver_user_id], asset_files=asset_files)
    if not query_id:
        print("Failed to send sign request")
        assert False
    
    ## wait_for_signs
    response_data = bbc_app_client.callback.sync_by_queryid(query_id, timeout=20)
    if response_data[KeyType.status] < ESUCCESS:
        print("Rejected because ", response_data[KeyType.reason].decode())
        assert False

    # result = response_data[KeyType.result]
    sig_receiver = response_data[KeyType.signature]
    source_id = response_data[KeyType.source_user_id]
    transaction.witness.add_signature(user_id=source_id, signature=sig_receiver)

    sig = transaction.sign(private_key=key_pair.private_key, public_key=key_pair.public_key)
    transaction.witness.add_signature(user_id=user_id, signature=sig)
    transaction.digest()

    # insert_signed_transaction
    query_id = bbc_app_client.insert_transaction(transaction)
    assert query_id
    if response_data[KeyType.status] < ESUCCESS:
        print("ERROR: ", response_data[KeyType.reason()].decode())
        assert False
    ...
    # send_transaction_info_msg
    transaction_info = [os.path.basename(filename), transaction.transaction_id]
    bbc_app_client.send_message(transaction_info, receiver_user_id)
