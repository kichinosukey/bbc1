...
user_id3 = bbclib.get_new_id("user_id3 for testing", include_timestamp=False)
...
def setup_client(user_id=None, domain_id=None, keypair=None, callback_obj=None, multiq=False, multicast=False):
...
def main(user_id=None, unicast_receiver_id_list=None, message=None):
    keypair = create_keypair()
    client = setup_client(user_id, domain_id, keypair, 
                    callback_obj=MessageProcessor(), multiq=True, multicast=False)

    txobj = make_transaction(user_id, asset_group_id, keypair)
    register_txobj(txobj, client)
    for i, unicast_receiver_id in enumerate(unicast_receiver_id_list):
        message_to_send = {"message": message+"_"+str(i)}
        client.send_message(message_to_send, unicast_receiver_id)
...
if __name__ == "__main__":
    ...
    if args[1] == "user1":
        user_id = user_id1
        message = args[2]
        main(user_id=user_id, unicast_receiver_id_list=[user_id2, user_id3], message=message)
