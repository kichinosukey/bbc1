...
def setup_client(user_id=None, domain_id=None, keypair=None, callback_obj=None, multiq=False, multicast=False, client=None):
    if client is None:
...
def main(user_id=None, multi_cast_user_id_list=None, message=None):
    ...
    for i, multi_cast_user_id in enumerate(multi_cast_user_id_list):
        client.send_message(message+"_"+str(i), multi_cast_user_id)
...
if __name__ == "__main__":
    ...
    if args[1] == "user1":
        user_id = user_id1
        message = args[2]
        main(user_id=user_id, multi_cast_user_id_list=[user_id2], message=message)
